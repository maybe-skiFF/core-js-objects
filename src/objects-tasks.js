...
function mergeObjects(objects) {
  ...
  objects.forEach((elem) => {
    const objLikeArr = Object.entries(elem);
    objLikeArr.forEach(([key, value]) => {
      if (mergeObj[key]) mergeObj[key] += value;
...
function makeWord(lettersObject) {
  const word = [];
  const objLikeArr = Object.entries(lettersObject);
  objLikeArr.forEach(([value, index]) => {
    index.forEach((place) => {
      word[place] = value;
    });
  });
...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    buffer += queue[i];
    if (buffer < queue[i + 1]) return false;
  }
  return true;
}
